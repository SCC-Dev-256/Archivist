...
class VODMetricsCollector:
    ...
    def _init_core_metrics(self):
        ...
        core_metrics = [
            (
                "vod_processing_total",
                MetricType.COUNTER,
                "Total VOD processing attempts",
            ),
            ...
            (
                "caption_generation_total",
                MetricType.COUNTER,
                "Total caption generation attempts",
            ),
            (
                "caption_generation_success",
                MetricType.COUNTER,
            ...
            (
                "video_retranscode_total",
                MetricType.COUNTER,
                "Total video retranscoding attempts",
            ),
            (
                "video_retranscode_success",
                MetricType.COUNTER,
                "Successful video retranscoding",
            ),
            (
                "video_retranscode_failed",
                MetricType.COUNTER,
                "Failed video retranscoding",
            ),
            ...
            (
                ...
                MetricType.COUNTER,
                "Total storage availability checks",
            ),
            (
                "storage_checks_failed",
                MetricType.COUNTER,
                "Failed storage availability checks",
            ),
            (
                "processing_duration",
                MetricType.HISTOGRAM,
                "VOD processing duration in seconds",
            ),
            (
                "download_duration",
                MetricType.HISTOGRAM,
                "VOD download duration in seconds",
            ),
            (
                "caption_duration",
                MetricType.HISTOGRAM,
                "Caption generation duration in seconds",
            ),
            (
                "retranscode_duration",
                MetricType.HISTOGRAM,
                "Video retranscoding duration in seconds",
            ),
        ...
        with self.lock:
            if metric_name not in self.metrics:
                self.metrics[metric_name] = Metric(
                    metric_name, MetricType.HISTOGRAM, ""
                )
    ...
    def get_metrics_summary(self, time_window: int = 3600) -> Dict[str, Any]:
        ...
        if "vod_processing_total" in summary and summary["vod_processing_total"] > 0:
            summary["error_rate"] = (
                summary.get("vod_processing_failed", 0)
                / summary["vod_processing_total"]
            ) * 100
        ...
        if "vod_download_retries" in summary and "vod_download_total" in summary:
            ...
            if total_downloads > 0:
                summary["retry_success_rate"] = (
                    (total_downloads - summary.get("vod_download_failed", 0))
                    / total_downloads
                ) * 100
    ...
    def collect_system_metrics(self):
        ...
        try:
            ...
            self.gauge(
                "system_process_memory_mb", process.memory_info().rss / (1024**2)
            )
...
def get_metrics_collector() -> VODMetricsCollector:
    ...
    if _metrics_collector is None:
        try:
            ...
            logger.warning(
                f"Redis not available for metrics, using in-memory only: {e}"
            )
