...
def setup_debug_logging():
    ...
    if DEBUG_MODE:
        ...
        debug_log_file = os.path.join(
            DEBUG_LOG_DIR,
            f'transcription_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log',
        )
...
def log_debug_info(context: str, data: Dict[str, Any]) -> None:
    ...
    if DEBUG_MODE:
        debug_file = os.path.join(
            DEBUG_LOG_DIR,
            f'debug_{context}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json',
        )
        try:
...
def get_system_info() -> Dict[str, Any]:
    ...
    return {
        ...
        "memory_available": (
            psutil.virtual_memory().available if "psutil" in sys.modules else None
        ),
        "disk_usage": (
            psutil.disk_usage("/")._asdict() if "psutil" in sys.modules else None
        ),
        "process_info": {
            "pid": os.getpid(),
            "ppid": os.getppid(),
...
def verify_file_operation(
    source_path: str, dest_path: str, operation: str = "copy"
) -> None:
    """Verify file operations (copy, move, write) were successful."""
    try:
        if not os.path.exists(dest_path):
            raise FileNotFoundError(
                f"Destination file not found after {operation}: {dest_path}"
            )
        ...
        if operation in ["copy", "move"]:
            ...
            if source_size != dest_size:
                raise IOError(
                    f"Size mismatch after {operation} (source: {source_size}, dest: {dest_size})"
                )

            # Verify file contents if it's a text file
            if dest_path.endswith(".srt"):
                with open(source_path, "r", encoding="utf-8") as f1, open(
                    dest_path, "r", encoding="utf-8"
                ) as f2:
...
def update_job_progress(
    current_job, progress: int, status: str, error: str = None
) -> None:
...
def run_whisper_transcription(video_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            if current_job:
                update_job_progress(
                    current_job, 70, "Transcription completed, processing segments..."
                )
        ...
        try:
            ...
            if current_job:
                update_job_progress(
                    current_job, 80, "Segments formatted, saving files..."
                )
        except Exception as e:
            if current_job:
        ...
        try:
            ...
            if current_job:
                update_job_progress(
                    current_job,
                    90,
                    "Local SRT file saved, copying to central location...",
                )
        except Exception as e:
            if current_job:
        ...
        try:
            ...
            if current_job:
                update_job_progress(
                    current_job,
                    100,
                    f"Completed! Generated transcript with {len(formatted_segments)} segments",
                )
        except Exception as e:
            if current_job:
                update_job_progress(
                    current_job, 0, "Failed to copy to central location", str(e)
                )
            raise
        ...
        try:
            ...
            # Trigger VOD publishing if enabled
            if os.getenv("AUTO_PUBLISH_TO_VOD", "false").lower() == "true":
                try:
                    from core.vod_automation import on_transcription_complete

                    on_transcription_complete(result.id)
                except Exception as e:
                    logger.error(f"Failed to auto-publish to VOD: {e}")
        except Exception as e:
        ...
        error_details = {
            ...
            "video_permissions": (
                oct(os.stat(video_path).st_mode)[-3:]
            ...
            "job_info": (
                {
                    ...
                    "start_time": (
                        current_job.meta.get("start_time") if current_job else None
                    ),
                    "progress": (
                        current_job.meta.get("progress") if current_job else None
                    ),
                }
                if current_job
                else None
