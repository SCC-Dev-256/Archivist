...
class CablecastAPIClient:
    ...
    def __init__(self, base_url: str = None, api_key: str = None):
        ...
        # Configure retries on the session using environment settings
        adapter = requests.adapters.HTTPAdapter(max_retries=VOD_MAX_RETRIES)
    ...
    def _make_request(self, method: str, endpoint: str, **kwargs) -> Optional[Dict]:
        ...
        for attempt in range(1, VOD_MAX_RETRIES + 1):
            try:
                ...
                if attempt == VOD_MAX_RETRIES:
                    logger.error("API request failed after maximum retries")
                    return None
                time.sleep(VOD_RETRY_DELAY)
    ...
    def get_vod_status(self, vod_id: int) -> Optional[Dict]:
        ...
        try:
            ...
            if response:
                logger.debug(
                    f"Retrieved VOD status for {vod_id}: {response.get('vodState')}"
                )
                return response
            return None
        except Exception as e:
        ...
        try:
            with open(file_path, "rb") as f:
                ...
                response = self._make_request(
                    "POST", f"/vods/{vod_id}/upload", files=files
                )
                if response is not None:
    ...
    def create_vod_chapter(self, vod_id: int, chapter_data: Dict) -> Optional[Dict]:
        ...
        try:
            response = self._make_request(
                "POST", f"/vods/{vod_id}/chapters", json=chapter_data
            )
            if response:
    ...
    def wait_for_vod_processing(
        self, vod_id: int, timeout: int = 1800, check_interval: int = 30
    ) -> bool:
        ...
        while time.time() - start_time < timeout:
            ...
            logger.info(
                f"VOD {vod_id} state: {vod_state}, progress: {percent_complete}%"
            )
